 ...
 *	- REMOVEINVALIDLOCALES: Remove locales that the user is not suppossed to save
 *
	...
	public function beforeValidate(Model $model, $options = array()) {
		...
		if((!isset($options['callbacks']) || $options['callbacks'] === true) && isset($model->data[$model->alias])){
			...
			if($user['User']['is_super']){
				...
				$languages = $this->getLanguageIds($model);
				$allowedLanguages = array_search($user['User']['language_id'], $languages);
			}
				...
				if(isset($data[$field])){
					...
					if(is_array($values)){
						foreach($values as $locale => $value){
							// REMOVEINVALIDLOCALES
							// Check if this is a super admin call or the locale is allowed for the user
							if(
								$allowedLanguages !== 'all' &&
								$locale !== $allowedLanguages
							){
								unset($model->data[$model->alias][$field][$locale]);
								continue;
							}
 ...
 * beforeSave callback
 * TASKS:
 *	- COPYLANGUAGES: Copy languages that are not currently translated (only on create)
 *
 * @param Model $Model
 * @return mixed
 */
	public function beforeSave(Model $Model, $options){
		// COPYLANGUAGES
		// Check if the current saving is a creation
		if(!isset($Model->data[$Model->alias][$Model->primaryKey])){
			// Get user 
			$user = $Model->getCurrentUser();
			// We do not need to do this for super admins, because they can copy and paste
			if(!$user['User']['is_super']){
				// Get all languages
				$languages = $this->getLanguageIds($Model);
				// Get current users' locale
				$userLocale = array_search($user['User']['language_id'], $languages);
				// Original model locale
				$originalModelLocale = (isset($Model->locale) ? $Model->locale : null);
				...
				foreach($this->runtime[$Model->alias]['fields'] as $field){
					...
					if(isset($data[$field][$userLocale])){
						// Check other languages for field
						foreach($languages as $locale => $languageId){
							// If language set for field, skip
							if(isset($data[$field][$locale])){
								continue;
							}
							// Deactivate all not translated entries
							if($field === 'active'){
								$data[$field][$locale] = Model::ACTIVE_INACTIVE;
							}
							else{
								$data[$field][$locale] = $data[$field][$userLocale];
							}
						}
					}
				}
				...
				// Reset Model locale
				$Model->locale = $originalModelLocale;

				// Set Model data to new data

				$Model->data[$Model->alias] = $data;
				$Model->Behaviors->Translate->beforeSave($Model, array('validate' => false));
			}
		}
 ...
 * @return array
 */
	public function getLanguageIds(Model $Model){
